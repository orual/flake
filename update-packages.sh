...
update_go_package() {
    ...
    local owner=$(grep -oP 'owner = "\K[^"]+' "$pkg_file" || echo "")
    local repo=$(grep -oP 'repo = "\K[^"]+' "$pkg_file" || echo "")
    ...
    # Get latest release
    local latest_tag=$(curl -s "https://api.github.com/repos/$owner/$repo/releases/latest" | jq -r .tag_name)
    ...
    if ! nix build -f "$SCRIPT_DIR" "$pkg_name" 2>&1 | tee /tmp/nix-build-output; then
        local correct_hash=$(grep -oP 'got:\s+\K[^\s]+' /tmp/nix-build-output | tail -1)
...
update_npm_package() {
    ...
    local npm_name=$(grep -oP 'registry\.npmjs\.org/\K[^/]+/[^/]+' "$pkg_file" | head -1 || echo "")
    if [[ -z "$npm_name" ]]; then
    ...
    local latest_version=$(npm view "$npm_name" version 2>/dev/null || echo "")
    ...
    if ! nix build -f "$SCRIPT_DIR" "$pkg_name" 2>&1 | tee /tmp/nix-build-output; then
        ...
        local url_hash=$(grep -oP 'got:\s+\K[^\s]+' /tmp/nix-build-output | head -1)
        local deps_hash=$(grep -oP 'got:\s+\K[^\s]+' /tmp/nix-build-output | tail -1)
...
list_packages() {
    ...
    for pkg in "$PKGS_DIR"/*.nix "$PKGS_DIR"/*/default.nix; do
        if [[ -f "$pkg" ]]; then
            local name=$(basename "$(dirname "$pkg")")
            if [[ "$name" == "pkgs" ]]; then
