...
with lib; let
  ...
  '';
  mkPromExporterAvahiService = name:
    mkPromAvahiService {
      inherit name;
      ...
      grafana = {
        publicDashboards = mkOption {
          ...
          default = {
            ...
            "orual-ops" = "bc53c5457dbd4705bb0d56e67f57408c";
          };
        };
      };
    };
    ...
    (mkMerge [
      ...
      (mkIf cfg.prometheusMdns.enable {
        ...
        services.avahi.extraServiceFiles = mkIf cfg.prometheusMdns.enable (attrsets.mapAttrs'
          (name: value: {
            name = "${name}-exporter";
            value = mkPromExporterAvahiService name;
          })
      ...
      (mkIf cfg.observer.enable
        (
          let
            ...
            tailscaleScrapeTargets = let
              mkHostExporters = instance: config: let
                mkExporter = service: exporter: {
            ...
            # in (mapAttrsToList mkScrapeConfig eclssHosts);
                      ...
                      }
                      {
                        ...
                        items =
                          [
                            ...
                            #   url = "http://bmc.pattern.home.${cfg.observer.rootDomain}";
                            # }
                            # {
                            #   title = "Rack #0 PDU #0";
                          ...
                          ++ (lists.optional cfg.observer.victoriametrics.enable {
                            ...
                            url = "https://pattern.home.${cfg.observer.rootDomain}:${toString cfg.observer.victoriametrics.port}";
                          });
                      }
                    ];
                  };
                  ...
                  in {
                    apple-health-ingester = {
                      ...
                      cmd = [
                        ...
                        "--influxdb.orgName=nonbinary.computer"
