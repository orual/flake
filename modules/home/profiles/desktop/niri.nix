  ...
  config = mkIf cfg.enable {
    ...
    programs.niri.settings = {
      ...
      layout = {
        ...
        struts.left = 64;
        struts.right = 64;
        struts.top = if cfg.enableTablet then -4 else 0;
        struts.bottom = if cfg.enableTablet then 8 else 0;
        # border = {
        #   enable = true;
        ...
        default-column-width = {
          proportion = if cfg.enableTablet then 2.0 / 3.0 else 1.0 / 2.5;
        };
        # fog of war
        # focus-ring = {
        #   # enable = true;
        ...
        default-column-display = if cfg.enableTablet then "tabbed" else "normal";

        tab-indicator = {
          position = "bottom";
          gaps-between-tabs = if cfg.enableTablet then 0 else 10;
          width = if cfg.enableTablet then 16 else 3;
          gap = if cfg.enableTablet then 7 else 8;
          corner-radius = if cfg.enableTablet then 4 else 2;
          active.color = "${colors.base0B}ff";
      ...
      switch-events =
        ...
        in
        {
          ...
          lid-close.action = sh "systemctl suspend";
        };
        ...
        lib.attrsets.mergeAttrsList [
          {
            ...
            "${Mod}+D".action = spawn (if cfg.cosmicOnNiri then "cosmic-launcher" else "fuzzel");
        ...
        [
          {
            ...
            geometry-corner-radius =
              let
                r = if cfg.enableTablet then 14.0 else 8.0;
              in
              {
                top-left = r;
                top-right = r;
          ...
          }
          {
            ...
            open-floating = true;
            default-column-width = {
              proportion = if cfg.enableTablet then 0.8 else 0.4;
            };
    ...
    programs.fuzzel = {
      ...
      settings = {
        main = {
          terminal = "ghostty";
          font = lib.mkForce (if cfg.enableTablet then "Ioskeley Mono:12" else "Ioskeley Mono:10");
        };
      };
    };
